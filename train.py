...
def main(opt):
    ...
    if resume_training:
        ...
        pretrained_dict = torch.load('checkpoints/fresh9_cont.pt', map_location='cuda:0' if cuda else 'cpu')
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=.00003)

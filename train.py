...
def main(opt):
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.checkpoint_interval == 0:
            torch.save(model.state_dict(), '%s/epoch21_%d_sgd_%g.pt' % (opt.checkpoint_dir, epoch, opt.img_size))

    # save final model
    torch.save(model.state_dict(), '%s/epoch_%d_sgd_%g.pt' % (opt.checkpoint_dir, epoch, opt.img_size))

...
parser.add_argument('-weights_path', type=str, default='checkpoints/test_model.pt', help='weights path')
...
def detect(opt):
    ...
    state = model2.state_dict()
    pretrained_dict = torch.load('checkpoints/test_model.pt', map_location='cuda:0' if cuda else 'cpu')
    # 1. filter out unnecessary keys
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if ((k in state) and (state[k].shape == v.shape))}
    # 2. overwrite entries in the existing state dict
    state.update(pretrained_dict)
